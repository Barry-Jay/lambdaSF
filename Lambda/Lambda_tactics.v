...
forall (M N : lambda), red M N -> forall (n k : nat), red  (lift_rec M n k) (lift_rec N n k).
...
Definition preserves_lift_rec (red: lambda ->  lambda -> Prop) := 
...
forall (M M' N : lambda), red M M' -> forall ( k : nat), red  (subst_rec M N k) (subst_rec M' N k).

Definition subst_rec_preserves_r (red: termred) := 
forall (M N N' : lambda), red N N' -> forall ( k : nat), red  (subst_rec M N k) (subst_rec M N' k).

Definition subst_rec_preserves (red: termred) := 
forall (M M' : lambda), red M M' -> forall N N', red N N' -> forall ( k : nat), red  (subst_rec M N k) (subst_rec M' N' k).
...
forall (M: lambda) (n k: nat), status (lift_rec M n k) = relocate (status M) (S n) k. 
Proof. 
cut(forall p (M: lambda) (n k: nat), p>= rank M -> status (lift_rec M n k) = relocate (status M) (S n) k). 
split_all. eapply2 H. 
...
omega. 
gen2_case IHM1 H l1; try (rewrite relocate_succ; auto); try (eapply2 IHM1); omega. 
omega. 
Qed. 
...
forall (M: lambda) (n k: nat), rank (lift_rec M n k) = rank M.
Proof. induction M; split_all. Qed. 

Lemma subst_rec_preserves_status: 
forall (M N: lambda)(k : nat), (status M <= k -> status (subst_rec M N k) = status M). 
Proof. 
cut(forall p (M: lambda), p>= rank M -> forall (N: lambda) (k : nat), (status M <= k -> status (subst_rec M N k) = status M)).
split_all. eapply2 H. 
induction p; intro M. split_all. 
assert(rank M > 0) by eapply2 rank_positive. 
noway. 
...
unfold subst_rec; fold subst_rec. 
(* 1 *) 
replace (App (subst_rec l3 N k) (subst_rec l4 N k)) with (subst_rec (App l3 l4) N k) by auto. 
eapply2 IHp. 
simpl in *; omega. 
Qed. 
