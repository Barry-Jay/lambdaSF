...
Lemma ref_irreducible : forall n, irreducible (Ref n) lambda_red1. 
Proof. intro n. red. split_all. inversion H; auto. Qed. 

Lemma active_irreducible : forall M, status M >0 ->  irreducible (left_component M) lambda_red1 ->  irreducible (right_component M) lambda_red1 -> irreducible M lambda_red1. 
Proof. 
...
eapply2 r. 
(* 2 *) 
assert(lambda_red1 (star M0) (star M')). 
eapply2 lambda_red1_preserves_star_active. omega. 
eapply2 r. 
(* 1 *) 
...
Lemma compound_irreducible : forall M, compound M ->  irreducible (left_component M) lambda_red1 ->  irreducible (right_component M) lambda_red1 -> irreducible M lambda_red1. 
Proof. 
...
inversion H0. 
inversion H3. 
assert(lambda_red1 (Abs (star M1)) (Abs (star M'0))). 
eapply2 abs_lambda_red. 
eapply2 lambda_red1_preserves_star_compound. 
eapply r. eexact H8. 
(* 2 *) 
subst; simpl in *. 
inversion H0.
...
forall M, irreducible M lambda_red1 ->  irreducible (Abs M) lambda_red1. 
Proof. red; split_all. inversion H0. eapply2 H. Qed. 



Lemma normal_is_irreducible: 
forall M, normal M -> irreducible M lambda_red1. 
Proof. 
intros M nor; induction nor; split_all.  
...
Theorem progress : 
forall (M : lambda), normal M \/ (exists N, lambda_red1 M N) .
Proof. 
...
Lemma normal_is_stable: forall M, normal M -> forall N, lambda_red M N -> N = M.
Proof. 
split_all. 
inversion H0; inv1 lambda_red. 
assert False by eapply2 normal_is_irreducible. noway. 
Qed. 

Lemma unf: forall M N P, lambda_red M N -> lambda_red M P -> normal N -> normal P -> N = P.
Proof.  
split_all. 
elim(diamond_lambda_red M N H P); split_all. 
