...
gen_case H0 n. 
(* 5 *) 
gen_case H0 N; try discriminate. 
(* 5 *) 
gen_case H0 n0; discriminate.
(* 4 *) 
gen_case H0 N. 
gen_case H0 n1. 
discriminate. 
...
discriminate. 
inversion H0. 
assert(M = l). eapply2 IHM. inversion H. auto. 
congruence. 
(* 1 *) 
gen2_case H H0 N. 
gen_case H0 n.
...
forall M N, maxvar M = 0 -> maxvar N = 0 -> normal M -> normal N -> 
left_component M = left_component N -> 
right_component M = right_component N -> M = N. 
Proof. 
induction M; split_all. 
(* 3 *) 
gen4_case H0 H2 H3 H4 N; try discriminate.  
subst. gen_case H4 l. gen_case H4 n. discriminate. 
subst. inversion H2. inversion H7. inversion H7. 
(* 2 *) 
gen4_case H0 H2 H3 H4 N; try discriminate.  
(* 4 *) 
gen_case H4 M. 
gen_case H4 n. discriminate. 
assert(M=l). eapply2 star_monotonic. inversion H1; auto. 
congruence. 
(* 2 *) 
subst. 
inversion H2. simpl in *. noway. 
inv1 compound. 
(* 1 *) 
...
Lemma  times_monotonic3:  forall m1 m2 n1 n2, m1 <= n1 -> m2 <= n2 -> m1 * m2 <= n1 * n2. 
Proof. 
split_all.  
replace n1 with (m1 + (n1 - m1)) by omega. 
replace n2 with (m2 + (n2 - m2)) by omega. 
rewrite times_distributive2.
assert(0<=  (n1 - m1) * m2 + m1 * (n2 - m2) + (n1 - m1) * (n2 - m2)) . 
case ((n1 - m1) * m2 + m1 * (n2 - m2) + (n1 - m1) * (n2 - m2)); split_all. omega.  
omega. 
Qed. 

Lemma  times_monotonic4:  forall m n2 n1, n1 <= n2 -> m * n1 <= m * n2. 
Proof. 
intros m n2; induction n2; split_all.  
assert(n1 = 0) by omega; subst.  omega. 
assert(n1 <= n2 \/ n1 = S n2) by omega. 
inversion H0. 
assert(m * n1 <= m * n2) by eapply2 IHn2. 
assert(m * S n2 =  m + m * n2) by ring. 
rewrite H3. omega. 
subst. 
omega. 
Qed. 
